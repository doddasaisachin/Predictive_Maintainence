...
def home():
    if request.method=='GET':
        ...
        def predict_machine_failure(model,Type,Air_temp,Process_temp,Rotational_speed,Torque,Tool_wear,TWF,HDF,PWF,OSF):
            if (TWF=='1') or (OSF=='1') or (HDF=='1') or (PWF=='1'): 
                return True
        ...
        if predicted_result is True:
            ...
            reason=''
            failed_array=[TWF,HDF,PWF,OSF]
            for i in range(len(failed_array)):
                if i==0 and failed_array[i]=='1':
                    reason+='"TWF" '
                if i==1 and failed_array[i]=='1':
                    reason+='"HDF" '
                if i==2 and failed_array[i]=='1':
                    reason+='"PWF" '
                if i==3 and failed_array[i]=='1':
                    reason+='"OSF" '
            if len(reason)>5:
                new_reason='Machine Failed due to '+reason+'Failure'

            return render_template('index.html',result=output,Reason=new_reason)
        else:
